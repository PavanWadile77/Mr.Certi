...
document.getElementById('bulkDelete')?.addEventListener('click',async()=>{const ids=[...document.querySelectorAll('.p-check:checked')].map(c=>c.dataset.id);if(!ids.length){toast('Select participants first','warning');return}if(!confirm(`Delete ${ids.length} selected participant(s)?`))return;let deleted=0;for(const id of ids){try{await deleteDoc(doc(db,'participants',id));deleted++;}catch(e){console.error(e);toast(`Error deleting some participants: ${e.message}`, 'error');}}toast(`Deleted ${deleted} participant(s)`,'success');await loadData();const selectAll = document.getElementById('selectAll');if(selectAll) selectAll.checked = false;});
...
const EMAILJS_SERVICE_ID = 'service_mrcerti14'; // Replace with your Service ID
const EMAILJS_TEMPLATE_ID = 'template_mrcerti7'; // Replace with your Template ID
const EMAILJS_PUBLIC_KEY = 'd0eRJ2yYf_e-ruGhy'; // Replace with your Public Key
// ===========================================
